...
def enrich_emails_with_hunter(input_file, enriched_output_file):
    ...
    if not api_key_manager.get_key():
        print("Warning: HUNTER_API_KEYS environment variable not set or empty. Skipping Hunter.io enrichment.")
        return

    # Define a ranking system for job titles. Lower score is better.
    priority_titles = {
        "founder": 1, "co-founder": 1, "cofounder": 1,
        "ceo": 2, "chief executive officer": 2, "cto": 3, "chief technology officer": 3,
        "coo": 4, "chief operating officer": 4, "cfo": 5, "chief financial officer": 5,
        "cio": 6, "chief information officer": 6, "cmo": 7, "chief marketing officer": 7,
        "cso": 8, "chief strategy officer": 8, "vp": 9, "vice president": 9,
        "head of": 10, "director": 11, "manager": 12
    }
    ...
    def get_contact_rank(position):
        ...
        for title, rank in priority_titles.items():
            if title in position_lower: return rank
        return 99
    ...
    try:
        ...
        for domain in unique_websites:
            if not api_key_manager.get_key():
                print("Stopping enrichment as all API keys have been exhausted.")
                break

            print(f"  -> Searching for contacts at: {domain}")
            hunter_contacts = get_hunter_contacts(domain, api_key_manager)
            ...
            if hunter_contacts:
                ...
                for contact in top_10_contacts:
                    top_contacts_per_domain.append({
                        'Website': domain, 'Found_Email': contact['email'],
                        'Found_Name': contact['name'], 'Position': contact.get('position', ''),
